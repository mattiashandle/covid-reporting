...
export class HealthcareProviderClient {
    ...
    updateOrderReport(healthcareProviderId: string, id: string, jsonPatchDocument: Operation[]): Promise<OrderReportDto> {
        let url_ = this.baseUrl + "/healtcareProvider/{healthcareProviderId}/orderReports/{id}";
        if (healthcareProviderId === undefined || healthcareProviderId === null)
            throw new Error("The parameter 'healthcareProviderId' must be defined.");
        url_ = url_.replace("{healthcareProviderId}", encodeURIComponent("" + healthcareProviderId));
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.processUpdateOrderReport(_response);
        });
    }

    protected processUpdateOrderReport(response: Response): Promise<OrderReportDto> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
